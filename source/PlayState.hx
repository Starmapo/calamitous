...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		//PLAYER
		uiSkin = UIData.getUIFile(SONG.uiSkin);
		if (uiSkin == null) {
		...
		if (maniaData == null) {
			FlxG.log.add('Couldn\'t get ' + opponentKeys + 'K data for ' + uiSkin.name + '!');
			var bad:SkinFile = UIData.getUIFile('default');
			maniaData = bad.mania[opponentKeys - 1];
		}
		...
		gf = new Character(0, 0, gfVersion, false, false, true);
		startCharacterPos(gf);
		gf.scrollFactor.set(0.95, 0.95);
		gfGroup.add(gf);
		startCharacterLua(gf.curCharacter);

		dad = new Character(0, 0, SONG.player2, false, opponentChart, true);
		...
		if (SONG.player2 == 'pshaggy'){
			...
			insert(members.indexOf(dadGroup) - 1, shaggyT);

			legT = new FlxTrail(legs, null, 5, 7, 0.3, 0.001);
			insert(members.indexOf(dadGroup) - 1, legT);
		}
		
		boyfriend = new Boyfriend(0, 0, SONG.player1, opponentChart);
		...
		timeBarBG = new AttachedSprite(UIData.checkImageFile('timeBar', uiSkin));
		...
		healthBarBG = new AttachedSprite(UIData.checkImageFile('healthBar', uiSkin));
		...
		#if LUA_ALLOWED
		var doPush:Bool = false;
		var luaFile:String = 'images/uiskins/' + uiSkin.name + '.lua';
		if(FileSystem.exists(Paths.modFolders(luaFile))) {
			luaFile = Paths.modFolders(luaFile);
			doPush = true;
		} else {
  ...
  public function clearShaderFromCamera(cam:String){
			...
			startTimer = new FlxTimer().start(modifiedCrochet / 1000, function(tmr:FlxTimer)
			{
				...
				for (boyfriend in boyfriendGroup) {
					if(tmr.loopsLeft % 2 == 0) {
						if (!boyfriend.stunned && boyfriend.animation.curAnim.name != null && !boyfriend.animation.curAnim.name.startsWith("sing")) {
							boyfriend.dance();
						}
					} else if (boyfriend.danceIdle && !boyfriend.stunned && boyfriend.animation.curAnim.name != null && !boyfriend.animation.curAnim.name.startsWith("sing")) {
						boyfriend.dance();
					}
				}
				...
				for (dad in dadGroup) {
					if(tmr.loopsLeft % 2 == 0) {
						if (!dad.stunned && dad.animation.curAnim.name != null && !dad.animation.curAnim.name.startsWith("sing")) {
							dad.dance();
						}
					} else if (dad.danceIdle && !dad.stunned && dad.animation.curAnim.name != null && !dad.animation.curAnim.name.startsWith("sing")) {
						dad.dance();
					}
				}
				...
				switch (swagCounter)
				{
					...
					case 1:
						countdownReady = new FlxSprite().loadGraphic(Paths.image(UIData.checkImageFile(introAlts[0], uiSkin)));
						countdownReady.scrollFactor.set();
						countdownReady.updateHitbox();

						countdownReady.setGraphicSize(Std.int(countdownReady.width * uiSkin.scale * uiSkin.countdownScale));
					...
					case 2:
						countdownSet = new FlxSprite().loadGraphic(Paths.image(UIData.checkImageFile(introAlts[1], uiSkin)));
						countdownSet.scrollFactor.set();

						countdownSet.setGraphicSize(Std.int(countdownSet.width * uiSkin.scale * uiSkin.countdownScale));
					...
					case 3:
						if (!skipCountdown){
							countdownGo = new FlxSprite().loadGraphic(Paths.image(UIData.checkImageFile(introAlts[2], uiSkin)));
							countdownGo.scrollFactor.set();

							countdownGo.setGraphicSize(Std.int(countdownGo.width * uiSkin.scale * uiSkin.countdownScale));
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, keys, uiSkin);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (curStepCrochet * susNote) + (curStepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, keys, uiSkin, curStepCrochet);
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...keys)
		{
			...
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player, keys, uiSkin);
			...
			if (!isStoryMode)
			{
				babyArrow.y -= 10;
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {y: babyArrow.y + 10, alpha: targetAlpha}, 1, {ease: FlxEase.circOut, startDelay: delay + (delay2 * i)});
			}
			else
			{
	...
	{
		if(FlxG.sound.music == null || startingSong || endingSong || finishTimer != null) return;
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'sky':
				...
				if (curBeat < 60)
					sh_r = 60;
				else if ((curBeat >= 300 && curBeat < 444) || curBeat >= 500)
					sh_r += (60 - sh_r) / 32;
				else
					sh_r = 600;
		...
		if(!inCutscene) {
			...
			if(!startingSong && !endingSong && playerChar.animation.curAnim.name.startsWith('idle')) {
		...
		if (generatedMusic && PlayState.SONG.notes[getCurSection()] != null && !endingSong && !isCameraOnForcedPos && curBeat >= 0)
		{
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image(UIData.checkImageFile(daRating, uiSkin)));
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(UIData.checkImageFile('combo', uiSkin)));
		...
		rating.setGraphicSize(Std.int(rating.width * uiSkin.scale * uiSkin.ratingScale));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(UIData.checkImageFile('num${Std.int(i)}', uiSkin)));
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		{
			if(!playerChar.stunned && generatedMusic && !endingSong)
			{
	...
	private function keyShit():Void
	{
		...
		if (!playerChar.stunned && generatedMusic)
		{
			// rewritten inputs???
			notes.forEachAlive(function(daNote:Note)
			{
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && char.animOffsets.exists('hey')) {
			...
			char.specialAnim = true;
			char.heyTimer = 0.6 / playbackRate;
		...
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				//if (note.isSustainNote){ wouldn't this be fun : P. i think it would be swell
					...
					//	if(char.animation.getByName(anim) == null)anim = animToPlay + daAlt;
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'oversinging':
						if(playerChar.holdTimer >= 10 && !usedPractice) {
							unlock = true;
						}
					case 'hype':
